...
function ClassicRules() {
  return (
    <>
      <RuleSection title="目標" accent="text-teal-700 dark:text-teal-400">
        <li>手牌固定 <strong>4 張數字牌</strong>（牌值 1–13），透過加減乘除組出等於 <strong>24</strong> 的算式</li>
        <li>持續出牌累積分數，直到<strong>牌庫抽完</strong>為止</li>
      </RuleSection>

      <RuleSection title="流程">
        <li>需<strong>一次用完</strong>手上全部 4 張數字牌，且算式結果等於 24 才能出牌得分</li>
        <li>答對後自動<strong>抽新牌補滿手牌</strong>，進入下一手</li>
        <li>若判斷目前手牌湊不出 24，可按「<strong>跳過換牌</strong>」放棄整手重新抽牌（系統會盡量確保抽到的新手牌有解）</li>
      ...
      <RuleSection title="完美手加成" accent="text-amber-700 dark:text-amber-400">
        <li>本手得分達到<strong>該手牌理論最高分</strong>（完美解）時，額外 <strong>+1 分</strong></li>
        <li>結算畫面會依「總分 ÷ 理論上限」換算<strong>解法效率</strong>，並給出 S / A / B / C 評級</li>
      </RuleSection>
    </>
  );
}
  ...
  return (
    <>
      <RuleSection title="目標" accent="text-amber-700 dark:text-amber-400">
        <li>倒數 <strong>5 分鐘</strong>內，使用 <strong>4 張數字牌</strong>透過加減乘除湊出等於 <strong>24</strong> 的算式</li>
        <li>盡可能撐過越多關卡，關卡數<strong>沒有上限</strong></li>
      </RuleSection>
      ...
      <RuleSection title="加時規則">
        <li>答對一題即<strong>加時</strong>並進入下一關</li>
        <li>第 1 關加 <strong>60 秒</strong>，之後每過一關遞減 3 秒，最低 <strong>15 秒</strong></li>
      </RuleSection>

      <RuleSection title="跳過懲罰" accent="text-rose-600 dark:text-rose-400">
        <li>覺得目前 4 張牌湊不出 24，可<strong>跳過換題</strong>，但會扣時</li>
        <li>同一關內第 1 次跳過扣 <strong>15 秒</strong>、第 2 次扣 <strong>30 秒</strong>、第 3 次起固定扣 <strong>60 秒</strong></li>
        <li>本關內只要答對一次，跳過次數即<strong>歸零</strong>重新計算</li>
      </RuleSection>

      <RuleSection title="結束條件">
        <li>倒數時間歸零，或主動按「<strong>提前結算</strong>」結束</li>
        <li>以「<strong>最終關卡</strong>」與「<strong>總分</strong>」計算成績，並記錄個人最佳關卡</li>
      </RuleSection>
    </>
  );
}
